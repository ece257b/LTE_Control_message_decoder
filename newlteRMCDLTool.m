...
function [waveform,frame,rmc] = newlteRMCDLTool(varargin)
...
if(isempty(varargin))
    ...
    for subframeIdx=NSubframe:(NSubframe+totalSubframes)-1
        ...
        if (info.NSymbolsDL)
            ...
            if (isfield(enbConfig,'SIB')) && (mod(subframeIdx,10) == 5) && (mod(SFN,2)==0)
                ...
                if any(strcmpi(enbConfig.SIB.Enable,{'On','Enable'}))                    
                    [subframe, prbsSIB, pdcchSymbolsSIB] = addSIB1(enbConfig,subframe);
                    % Avoid OCNG overwriting SIB PDSCH by adding it to
                    % 'prbs' (these RBs are avoided when adding OCNG)
                    prbs = vertcat(prbs, prbsSIB); %#ok<AGROW>
                    prbs = sort(prbs); % sort in ascending order
                    % Avoid OCNG overwriting PDCCH for SIB DCI by adding it
                    % to 'pdcchSymbols' (these symbols are avoided when
                    % adding OCNG)
                    if (isempty(pdcchSymbols))
                        pdcchSymbols = pdcchSymbolsSIB;
                    else
                        pdcchSymbols = pdcchSymbols + pdcchSymbolsSIB;
                    end
                end
            end
        ...
        if(mod(count,10)==0)
            [subwaveform,ofdmInfo] = lteOFDMModulate(enbConfig,frame,enbConfig.Windowing,enbConfig.Nfft);
            if isempty(waveform)
                waveform = subwaveform;
            else
                waveform=cat(2,waveform,subwaveform);
            end
            frame = [];
        end
        if(mod(subframeIdx,100)==0)
            fprintf("%d\n", count);
        end
    end
...
function completeStruct = checkFields(enbConfig)
    ...
    fields1 = isfield(enbConfig,{'NDLRB','CellRefP','NCellID','CyclicPrefix',...
        'CFI','PCFICHPower','Ng','PHICHDuration','HISet','PHICHPower','NFrame','NSubframe','TotSubframes','Windowing',...
        'Nfft','DuplexMode','PDSCH','OCNGPDCCHEnable','OCNGPDCCHPower','OCNGPDSCHEnable','OCNGPDSCHPower','OCNGPDSCH',});
...
function [subframe,PRBSet,pdcchSymbols] = addSIB1(enbConfig,subframe)
