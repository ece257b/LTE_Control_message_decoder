...
if loadFromFile
    filename = '/home/jason/Downloads/R2_RMC.mat';
    load(filename)           % Load I/Q capture of eNodeB output
%     eNodeBOutput = double(waveStruct.waveform)/32768; % Scale samples
    eNodeBOutput = double(waveStruct.waveform);
...
%     rmc.SIB.Data = randi([0 1],176,1);
    ...
    [eNodeBOutput,ogrid,info] = lteRMCDLTool(cfg,[1;0;0;1]);
...
% if (size(rxgrid,2)>=(L*2))
%         rxgrid(:,1:(L*1),:) = [];   % Remove subframes 0 to 4        
% else    
%     rxgrid = [];
% end
% enb.NSubframe = 0;
